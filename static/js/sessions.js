...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'Lodestar Chat';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Lodestar kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'Lodestar Chat';
    }
