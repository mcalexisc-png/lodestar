...
async def _call_teacher(teacher_model_spec: str, prompt: str,
    ...
    try:
        url, model, headers = await _resolve_model(teacher_model_spec, owner=owner)
...
async def run_teacher_inline(
    *,
    ...
    try:
        ...
        teacher_url, teacher_model, teacher_headers = await _resolve_model(teacher_spec, owner=owner)
    except Exception as e:
