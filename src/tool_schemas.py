...
FUNCTION_TOOL_SCHEMAS = [
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Manage calendar events: list events in a date range, create, update, delete. Each event can carry a tag/category (event_type) and importance level. Resolve relative dates like today/tomorrow against the 'Current date and time' system context, then pass ISO 8601 datetimes in the user's local wall time; for all-day events set all_day=true and pass YYYY-MM-DD. For event reminders/alarms, pass reminder_minutes; the tool creates the Lodestar note reminder, so do not also call manage_notes for the same reminder.",
            "parameters": {
                "type": "object",
                "properties": {
                    "action": {"type": "string",
                    ...
                    "reminder_minutes": {"type": "integer", "description": "For create_event: create an Lodestar reminder this many minutes before the event, e.g. 5 for 'reminder 5 min before'."},
...
def function_call_to_tool_block(name: str, arguments: str) -> Optional[ToolBlock]:
    ...
    if tool_type not in TOOL_TAGS:
        # In-process plugin tools (Tier 2) are discovered at runtime and are not
        # in the static TOOL_TAGS set. Pass their full JSON args through as
        # content; the plugin parses what it needs. _try_plugin_dispatch in
        # tool_execution rejects names that aren't actually registered/enabled.
        try:
            from src.plugins import get_plugin_loader

            if name in get_plugin_loader().tool_names():
                return ToolBlock(name, json.dumps(args) if args else "{}")
        except Exception:
            pass
