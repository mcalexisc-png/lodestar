#!/bin/bash
# Build a downloadable macOS launcher app + .dmg for Lodestar.
#
#   ./build-macos-app.sh
#
# Produces:
#   dist/Lodestar.app   — double-click: starts the local server (using this
#                         repo's venv) and opens the UI in an app-style window.
#   dist/Lodestar.dmg   — drag-to-Applications disk image (the downloadable).
#
...
#!/bin/bash
# Lodestar.app — start the local server and open the UI in an app window.
...
die_gui() {
  /usr/bin/osascript -e "display dialog \"$1\" with title \"Lodestar\" buttons {\"OK\"} default button 1 with icon stop" >/dev/null 2>&1
  exit 1
}

[ -x "$UVICORN" ] || die_gui "Lodestar isn't set up yet. Open Terminal and run:
...
for i in $(seq 1 120); do
  ...
  kill -0 "$SERVER_PID" 2>/dev/null || die_gui "Lodestar failed to start. Log:
$LOG"
  sleep 1
done
...
if [ "$READY" = "1" ]; then
  ...
  notify "Lodestar is taking a while — open $URL once it finishes starting."
fi
wait "$SERVER_PID"
LAUNCHER
...
echo "Install:       open '$DIST/$APP_NAME.dmg'  (drag Lodestar to Applications)"
